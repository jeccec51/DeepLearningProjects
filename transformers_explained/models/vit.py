...
class VisionTransformerBackbone(nn.Module):
    ...
    def forward(self, in_tensor: torch.Tensor) -> torch.Tensor:
        """Forward pass for vision transformer backbone.
        ...
        for layer in self.encoder_layers:
             # Register hook to save attention weights
            layer.self_attn.register_forward_hook(self.save_attention_weights)
            # Layer normalization before self-attention
            norm_embeddings = layer.norm1(embeddings)
            # Multi-head self-attention
            attention_output, _ = layer.self_attn(norm_embeddings, norm_embeddings, norm_embeddings)
